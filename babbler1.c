...
int makeTokens(lua_State *L){
  ...
  int token_len = end - offset;
  char token[token_len + 1];
  memcpy(token, &filename[offset], token_len);
  token[token_len] = '\0';
...
int main(int argc, char const * argv[]) {
  ...
  lua_register(L, "cmakeTokens", makeTokens);

  if (luaL_dofile(L, "babbler1.lua")) {
